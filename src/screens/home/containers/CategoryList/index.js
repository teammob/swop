...
class CategoryList extends Component {
  render() {
    ...
    let widthImage =
      fields.width && parseInt(fields.width) ? parseInt(fields.width) : 109;
    let heightImage =
      fields.height && parseInt(fields.height) ? parseInt(fields.height) : 109;

    const dataParent = category.data.filter(item => item.parent === 0);

    const limit =
      fields.limit && parseInt(fields.limit)
